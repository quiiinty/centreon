...
it(
    ...
    fn(string $json, array $expected) => expect((new FeatureFlags(false, $json))->getFlags())
        ->toBe($expected)
)->with([
    ['not-a-valid-json', []],
    ['{"foo": 0}', ['foo' => false]],
...
it(
    ...
    fn(string $json, array $expected) => expect((new FeatureFlags(true, $json))->getFlags())
        ->toBe($expected)
)->with([
    ['not-a-valid-json', []],
    ['{"foo": 0}', ['foo' => false]],
...
it(
    ...
    fn(string $json) => expect((new FeatureFlags(false, $json))->getFlags())->toBe([])
)->with([
    ['{"foo": null}'],
    ['{"foo": true}'],
    ['{"foo": 3.14}'],
...
it(
    ...
    fn(int $int) => expect((new FeatureFlags(false, '{"' . $int . '": 1, "i' . $int . '": 1}'))->getFlags())
        ->toBe(['i' . $int => true])
)->with([
    [0],
    [42],
